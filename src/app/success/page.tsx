...
function SuccessContent() {
  ...
  if (isPublic) {
    return <PublicSuccessContent sessionId={sessionId} />;
  }

  return <AuthenticatedSuccessContent sessionId={sessionId} />;
}

function AuthenticatedSuccessContent({ sessionId }: { sessionId: string }) {
...
function PublicSuccessContent({ sessionId }: { sessionId: string }) {
  ...
  useEffect(() => {
    async function verify() {
      try {
        const result = await apiClient<PublicVerifyResponse>(
          "/api/payment/verify-public",
          {
            method: "POST",
    ...
    <div className="flex min-h-screen items-center justify-center p-4">
      <Card className="w-full max-w-md text-center">
        ...
        <CardContent className="space-y-2">
          <p className="text-muted-foreground">
            {data?.recipientName
              ? `Your payment to ${data.recipientName} has been confirmed.`
